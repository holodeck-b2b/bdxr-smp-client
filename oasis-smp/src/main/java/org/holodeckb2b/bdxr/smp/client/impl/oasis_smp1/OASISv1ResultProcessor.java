 ...
 * <p><b>NOTE:</b> The specification states that the <code>//Redirect/CertificateUID</code> element holds the <i>Subject
 * Unique Identifier</i> of the certificate of the new SMP server. It however defines this field's type as
 * <code>xsd:string</code> without any specification how to convert this to the bit string value of the certificate
 * field. The example in the specification also shows the value to be a normal text, which is unexpected content for a
 * bit string. Because the handling of this element is unclear it is ignored by this processor.
 *
...
public class OASISv1ResultProcessor implements ISMPResultProcessor {
    ...
    static {
        try {
			...
			smpSchema = sf.newSchema(OASISv1ResultProcessor.class.getResource("/xsd/bdx-smp-201605.xsd"));
    ...
    public QueryResult processResult(Document xmlDocument) throws SMPQueryException {
        ...
        try {
            ...
            throw new SMPQueryException("XML could not be parsed as valid SMP result");
        }
	...
	private ServiceMetadata processServiceMetadata(ServiceMetadataType smdXML) throws SMPQueryException {
		...
		if (smdXML.getRedirect() != null) {
			 ...
			 * data element. In OASIS V1 however the redirection is on the service level. This implies that we need to
			...
			/* Convert the list of ProcessList/Process elements. Because in the OASIS V1 spec each process in which the
