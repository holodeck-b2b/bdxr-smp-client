...
public class eSENSHostNameGenerator implements IHostNameGenerator {
    ...
    public String getHostNameForParticipant(Identifier participantId) {
        ...
        if (!allowedSchemes.contains(schemeId))
            throw new IllegalArgumentException(schemeId + " is not a valid naming scheme according to e-SENS specs");

        final String canonicalId = "urn:oasis:tc:ebcore:partyid-type:" + schemeId + ":" + participantId.getValue();
